...
class Lotacao < ActiveRecord::Base
  ...
  #validates_uniqueness_of :orgao_id,:on=>:create,:scope=>[:funcionario_id,:data_lotacao],:message=>"Funcionário precisa ser devolvido para ser lotado novamente.",:if=>Proc.new{|l|l.tipo_lotacao=="SUMARIA ESPECIAL" or l.tipo_lotacao=="ESPECIAL"}
  ...
  def validar_complementar
    ...
    elsif self.funcionario.lotacoes.ativas.where(:destino_type=>"Departamento").any? or self.funcionario.lotacoes.ativas.where(:destino_type=>"Orgao").any?
