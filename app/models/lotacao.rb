...
class Lotacao < ActiveRecord::Base
  ...
  validates_uniqueness_of :orgao_id,:on=>:create,:scope=>[:funcionario_id,:data_lotacao],:message=>"Funcionário precisa ser devolvido para ser lotado novamente.",:on=>:create,:if=>Proc.new{|l|l.tipo_lotacao=="SUMARIA ESPECIAL" or l.tipo_lotacao=="ESPECIAL"}
