...
class PontosController < ApplicationController
  ...
  def funcionarios
    ...
    if @tipo=="departamento"
      ...
      @funcionarios = @objeto.funcionarios.joins(:pessoa,:lotacoes).uniq.where("lotacaos.finalizada = ? and lotacaos.data_lotacao <= ? and lotacaos.data_devolucao is null",true,@data.end_of_month).sort_by{|f|f.pessoa.nome}.paginate :page => params[:page], :per_page => 8
    end
